...
public class InformePdfUno extends DocumentoPdf {

	private static Logger log = Logger.getLogger(DocumentoPdf.class);

	private static String FILE = PATH_DOCUMENTO_PDF + "PdfPunto1.pdf";
	...
	private static void addCabecera(Document document) throws DocumentException {
		...
		preface2.add(new Chunk("EMPLEADOS SEGUN SALARIO", tituloAzul15));
		preface2.add(new Paragraph("Description:", description10));
		preface2.add(new Paragraph(
				"Registros de todos los empleados que cobran un salario mayor al salario promedio de su departamento. ",
				description10));
	...
	private static void createTable(Document document, List<EmpleadoDTO> lista) throws DocumentException {
		...
		cell = new PdfPCell(new Phrase("NOMBRE",confCell));
		cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		table.addCell(cell);

		cell = new PdfPCell(new Phrase("SALARIO",confCell));
		cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		table.addCell(cell);
		...
		for (EmpleadoDTO empleado : lista) {
			...
			table.addCell(Utils.formatearSalario(empleado.getSalario()));
		}
	...
	private static void addPieDePagina(Document document, List<EmpleadoDTO> lista) throws DocumentException {
		Paragraph preface2 = new Paragraph();
		preface2.add(new Paragraph(
				"Nombre del Departamento: " + lista.get(1).getDepartamento().getNombre().replaceAll("[\\(\\)\\[\\]\\{\\}]", "")));
